...
export default function ExploreScreen() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.error("Error cargando clases en tiempo real:", error);
        setIsLoadingClasses(false);
      },
    );
    ...
    <View className="flex-1 bg-impulse-dark pt-14 px-5">
      <View className="mb-6 mt-2 flex-row justify-between items-start">
        ...
        >
          ...
          <Ionicons name="qr-code-outline" size={18} color="#00E5FF" />
        ...
        {activeTab === "classes" ? (
          <View>
            ...
            {isLoadingClasses ? (
              ...
              <View className="py-10 items-center justify-center bg-[#1C1C1E] rounded-3xl mx-1 border border-white/5">
                {/* CORRECCIÓN: calendar.badge.exclamationmark -> alert-circle-outline */}
                ...
                return (
                  ...
                  >
                    <View className="flex-row justify-between items-center mb-4">
                      <View>
                        <Text
                          className={`text-3xl font-black tracking-tighter ${isCancelled ? "text-red-500/50" : "text-white"}`}
                        >
                          {cls.startTime}
                        </Text>
                        {cls.endTime && (
                        ...
                        <Text
                          className={`font-bold text-sm uppercase tracking-widest mt-1 ${isCancelled ? "text-red-400" : "text-gray-400"}`}
                        >
                          {cls.name}
                        </Text>
                      </View>
                    ...
                    <View className="flex-row items-center mb-5">
                      <View
                        className={`w-8 h-8 rounded-full items-center justify-center mr-3 ${isCancelled ? "bg-red-500/20" : "bg-white/10"}`}
                      >
                        {/* CORRECCIÓN: person.fill -> person */}
                        <Ionicons
                          name="person"
                          size={14}
                          color={isCancelled ? "#EF4444" : "#888"}
                        />
                      </View>
                      <Text
                        className={`${isCancelled ? "text-red-400" : "text-gray-300"} font-medium`}
                      >
                    ...
                    <View
                      className={`flex-row items-center justify-between mt-2 pt-4 border-t ${isCancelled ? "border-red-500/20" : "border-white/5"}`}
                    >
                      {isCancelled ? (
                        ...
                        <>
                          <Text
                            className={`text-xs font-black ${isFull ? "text-red-500" : "text-gray-400"}`}
                          >
                          ...
                          {isReserved ? (
                            <View className="flex-row gap-2">
                              ...
                              >
                                <Text
                                  className={`font-black text-xs ${isAttended ? "text-green-500" : "text-red-500"}`}
                                >
                            ...
                            >
                              <Text
                                className={`font-black text-xs ${isFull ? "text-white/20" : "text-black"}`}
                              >
                                RESERVAR
                              </Text>
                            </TouchableOpacity>
          ...
          <View>
            ...
            {isLoadingCoaches ? (
              ...
              <View className="py-10 items-center justify-center bg-[#1C1C1E] rounded-3xl mx-1 border border-white/5">
                {/* CORRECCIÓN: person.3.fill -> people */}
