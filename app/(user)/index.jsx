...
export default function UserDashboard() {
  ...
  const fetchEvaluations = async () => {
    ...
    try {
      ...
      const loadedEvals = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      ...
      loadedEvals.sort((a, b) => {
        const dateA = new Date(
          `${a.classDate || a.date}T${a.startTime || "00:00"}`,
        );
        const dateB = new Date(
          `${b.classDate || b.date}T${b.startTime || "00:00"}`,
        );
        return dateB - dateA;
      });
  ...
  const HabitIndicator = ({ label, score }) => {
    // CORRECCIÓN: Nombres de iconos universales
          ...
          {isLoading ? (
            ...
            <View className="bg-impulse-gray p-8 rounded-[32px] border border-white/5 items-center mt-2">
              ...
              <Text className="text-gray-500 text-xs text-center mt-2">
                Asiste a tu primera clase para que tu coach evalúe tu
                rendimiento y empieza a construir tu racha.
              </Text>
            </View>
          ) : (
            evaluations.map((evalData) => {
              ...
              return (
                <View
                  key={evalData.id}
                  className="bg-impulse-gray p-5 rounded-3xl mb-4 border border-white/5 shadow-xl"
                >
                  <View className="flex-row justify-between items-start mb-5 border-b border-white/5 pb-4">
                    <View className="flex-1">
                      ...
                      <Text className="text-[#00E5FF] text-[10px] font-bold uppercase tracking-widest">
                        {evalData.classDate || evalData.date} •{" "}
                        {evalData.startTime}
                      </Text>
                    </View>
                    <View
                      className={`px-3 py-1.5 rounded-full border ${badge.bg} ${badge.border}`}
                    >
                      <Text
                        className={`text-[10px] font-black tracking-widest uppercase ${badge.text}`}
                      >
                  ...
                  <View className="flex-row items-center justify-between">
                    <View className="flex-1 pr-4">
                      <HabitIndicator
                        ...
                        score={evalData.evaluation?.rutina}
                      />
                      <HabitIndicator
                        label="Cardio realizado"
                        score={evalData.evaluation?.cardio}
                      />
                      <HabitIndicator
                        label="Técnica / Indicaciones"
                        score={evalData.evaluation?.indicaciones}
                      />
                    </View>
