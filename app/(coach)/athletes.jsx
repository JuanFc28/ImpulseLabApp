    ...
    <SafeAreaView className="flex-1 bg-impulse-dark">
      <View className="flex-1 px-5 pt-6">
        ...
        {isLoading ? (
          ...
          <View className="flex-1 justify-center items-center">
            {/* CORRECCIÓN: person.crop.circle.badge.xmark -> person-remove-outline */}
            <Ionicons name="person-remove-outline" size={48} color="#444" />
          ...
          >
            {athletes.map((user) => (
              <View
                key={user.id}
                className="bg-impulse-gray p-4 rounded-3xl mb-3 border border-white/5 flex-row items-center"
              >
                {/* AVATAR */}
                <View
                  className="w-12 h-12 rounded-full items-center justify-center mr-4 border"
                  style={{
                    backgroundColor: athleteStyle.bg,
                ...
                >
                  <Text
                    ...
                    style={{ color: athleteStyle.color }}
                  >
                    {athleteStyle.text}
                  </Text>
                </View>
