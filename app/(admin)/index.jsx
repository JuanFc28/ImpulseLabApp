...
<<<<<<< HEAD
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const totalAthletes = usersSnap.docs.filter(
        (d) => d.data().role === "user",
      ).length;
      ...
      const barLabels =
        top4.length > 0 ? top4.map((c) => c.name.substring(0, 6)) : ["N/A"];
      ...
      if (totalRes > 0) {
        lineData = [
          ...
          Math.floor(totalRes * 0.25),
          totalRes -
            (Math.floor(totalRes * 0.15) + Math.floor(totalRes * 0.25) * 2),
        ];
      }
...
=======
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const usersDocs = getSafeDocs(results[0]);
            const classesDocs = getSafeDocs(results[1]);
            const reservationsDocs = getSafeDocs(results[2]);

            const totalAthletes = usersDocs.filter((docItem) => docItem.data()?.role === "user").length;

            const activeClasses = classesDocs
                .map((docItem) => ({
                    id: docItem.id,
                    ...docItem.data(),
                }))
                .filter((classItem) => classItem.active !== false);

            const totalClasses = activeClasses.length;

            const reservations = reservationsDocs.map((docItem) => ({
                id: docItem.id,
                ...docItem.data(),
            }));

            const totalRes = reservations.length;
            const attended = reservations.filter((reservation) => reservation.status === "attended").length;
            const pending = totalRes - attended;
            ...
            reservations.forEach((reservation) => {
                const className = reservation.className || reservation.name;
            ...
            const top4 = sortedClasses.slice(0, 4);
            const barLabels = top4.length > 0 ? top4.map((item) => item.name.substring(0, 6)) : ["N/A"];
            const barData = top4.length > 0 ? top4.map((item) => item.count) : [0];
    ...
    if (isLoading) {
        return (
            <View className="flex-1 bg-impulse-dark justify-center items-center">
                ...
                <Text className="text-gray-500 font-bold tracking-widest text-[10px] mt-4 uppercase">
                    Cargando métricas...
                </Text>
            </View>
        );
            ...
            >
                ...
                <View className="flex-row px-3.5 mb-6">
                    ...
                    <View className="flex-1 bg-impulse-gray m-1.5 p-4 rounded-3xl border border-white/5">
                        ...
                        <Text className="text-gray-500 text-[10px] font-bold uppercase">Clases Activas</Text>
                    </View>
                </View>
                ...
                <View className="flex-row px-5 mb-6">
                    ...
                    <View className="flex-1 bg-impulse-gray p-4 rounded-[32px] border border-white/5 ml-2">
                        ...
                        {stats.topRanking.length === 0 ? (
                            ...
                            stats.topRanking.map((item, index) => (
                                <View key={`${item.name}-${index}`} className="flex-row justify-between items-center mb-3">
                                    <View className="flex-row items-center flex-1">
                                        <Text className="text-emerald-500 font-black mr-2">{index + 1}</Text>
                                        <Text className="text-white text-[10px] font-bold flex-1" numberOfLines={1}>
                                            {item.name}
                                        </Text>
                                    </View>

                                    <Text className="text-gray-400 text-[10px]">{item.count}</Text>
                                </View>
                            ))
                        )}
                    </View>
    ...
    <SafeAreaView className="flex-1 bg-impulse-dark">
      ...
      >
        ...
        <View className="flex-row px-3.5 mb-6">
          ...
          <View className="flex-1 bg-impulse-gray m-1.5 p-4 rounded-3xl border border-white/5">
            {/* "calendar" es compatible con Ionicons */}
        ...
        <View className="flex-row px-5 mb-6">
          {/* GRÁFICA DE PASTEL */}
          <View className="flex-1 bg-impulse-gray p-4 rounded-[32px] border border-white/5 items-center mr-2">
            <Text className="text-white text-xs font-black self-start mb-2">
              Asistencias
            </Text>
            <PieChart
              data={stats.pieData}
          ...
          {/* RANKING TOP 3 */}
          <View className="flex-1 bg-impulse-gray p-4 rounded-[32px] border border-white/5 ml-2">
            <Text className="text-white text-xs font-black mb-4">
              Top 3 Clases
            </Text>
            {stats.topRanking.length === 0 ? (
              <Text className="text-gray-500 text-[10px]">Sin datos</Text>
              ...
              stats.topRanking.map((c, i) => (
                ...
                >
                  <View className="flex-row items-center">
                    <Text className="text-emerald-500 font-black mr-2">
                      {i + 1}
                    </Text>
                    <Text
